...
class AutokeyAlgorithm(CryptographicAlgorithm):
    def __init__(self):
        super().__init__()
        script_path = os.path.dirname(__file__)
        testing_directory_path = f"../../../../testing/symmetric_algorithms/substitution_algorithms/polyalphabetic_algorithms/autokey_algorithm"
    ...
    def get_key(self):
        ...
        with open(file_path, "r", encoding="utf-8") as file:
            initial_key = file.read().strip().lower() + self.plain_text
        final_key = ""
        for char_index in range(len(self.plain_text)):
            final_key += initial_key[char_index]
